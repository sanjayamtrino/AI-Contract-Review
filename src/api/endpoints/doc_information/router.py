"""Document information endpoints — summary and key details extraction."""
...
async def get_information(session_id: str = Depends(get_session_id)) -> Dict[str, Any]:
    ...
    return {"summary": result, "session_id": session_id}


@router.get("/key-information")
async def get_key_details(session_id: str = Depends(get_session_id)) -> Dict[str, Any]:
    """Extract structured key information from the ingested document."""
    try:
        result = await get_key_information(session_id=session_id, response_format="BaseModel")
    except ValueError as err:
        return {"error": str(err), "session_id": session_id}

    return {"Key Information": result, "session_id": session_id}
