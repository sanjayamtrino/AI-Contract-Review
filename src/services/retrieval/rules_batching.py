...
from src.schemas.rule_check import ParaSimilarity, RuleCheckRequest, RuleResult, TextInfo
...
def find_similarity(
    ...
    Iterates ALL paragraphs sorted by score descending.
    threshold is the only inclusion gate.
    top_k limits results sent to the LLM — not candidates evaluated.
    ...
    logger.info(f"Found {len(results)} paragraphs with similarity " f"above threshold of {threshold}.")
    return results
