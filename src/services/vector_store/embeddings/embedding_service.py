"""
Embedding services — HuggingFace (Sentence Transformers) and BGE implementations.

HuggingFaceEmbeddingService is the primary service used in production.
BGEEmbeddingService is an alternative with higher accuracy but more resource-intensive.
"""
...
class HuggingFaceEmbeddingService(BaseEmbeddingService, Logger):
    """Embedding service using Sentence Transformers (MiniLM default)."""
    ...
    async def get_health_status(self) -> Dict[str, Any]:
        """Run a test embedding and report health."""
        ...
        try:
            start_time = time.time()
            test_embedding = await self.generate_embeddings(text="Health check")
            health_stats.update({
...
class BGEEmbeddingService(Logger):
    """BGE embedding service (BAAI/bge-large-en-v1.5) — higher accuracy, more resources."""
    ...
    async def generate_embeddings(self, text: str, task: Optional[str] = "") -> List[float]:
        ...
        try:
            ...
            self.logger.debug(f"Generated BGE embedding in {generation_time:.4f}s")
    ...
    def _embed_text(self, text: str) -> torch.Tensor:
        """Synchronous embedding with mean pooling and L2 normalization."""
    ...
    async def get_health_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"healthy": True, "model": self.model_name, "embedding_dimension": len(embedding)}
        except Exception as e:
