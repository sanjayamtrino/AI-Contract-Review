...
def _validate_draft_response(
    ...
    *,
    ...
    if forbid_placeholders and found_placeholders:
        raise ValueError(f"Version: drafted_clause must not contain [PLACEHOLDER] tokens " f"when a document is attached (found {found_placeholders[:3]})")
    version.placeholders = found_placeholders


def _validate_regenerated_draft_differs(new_version: ClauseVersion, prior_version: ClauseVersion) -> None:
    """Regenerate must produce a draft meaningfully different from the prior one."""
    if new_version.drafted_clause.strip() == prior_version.drafted_clause.strip() or new_version.summary.strip() == prior_version.summary.strip():
        raise ValueError("Regenerated version is identical to the prior draft — not a meaningful variation")
...
def _validate_clause_list(
    ...
    *,
    ...
    for i, clause in enumerate(response.clauses):
        ...
        if forbid_placeholders and found_placeholders:
            raise ValueError(f"Clause {idx} ('{clause.title}'): drafted_clause must not contain " f"[PLACEHOLDER] tokens when a document is attached " f"(found {found_placeholders[:3]})")
    ...
    if avg_body_len < _MIN_LIST_AVG_CLAUSE_BODY_LEN:
        logger.info(
            "describe_draft list mode below recommended depth: avg=%.0f chars " "across %d clauses (recommended ≥%d). Returning anyway — user can " "regenerate individual clauses for more depth.",
            avg_body_len,
...
async def _get_doc_grounding(session_id: str) -> Optional[Dict[str, Any]]:
    ...
    from src.tools.key_information import (
        get_key_information,  # local import to avoid cycles
    )
    ...
    try:
        ...
        logger.warning("doc grounding extraction failed session=%s error=%s", session_id, str(e))
        return None
...
async def _retrieve_relevant_chunks(session_id: str, query: str, top_k: int = _RETRIEVAL_TOP_K) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.warning("doc retrieval failed session=%s error=%s", session_id, str(e))
        return []
...
async def _check_duplicate_clause(user_prompt: str, chunks: List[Dict[str, Any]]) -> Optional[ExistingClauseMatch]:
    ...
    for rank, chunk in enumerate(chunks[:_DUPLICATE_CHECK_TOP_N]):
        ...
        try:
            result = await container.azure_openai_model.generate(
                ...
                system_message=(
                    "You decide whether a candidate passage — which may be a full " "clause or only an excerpt — is on the same topic as the user's " "drafting request. Return ONLY valid JSON."
                ),
                temperature=0.0,
            )
        except Exception as e:
...
def _find_clause_in_list(clauses: List[ClauseListEntry], title: str) -> Tuple[Optional[int], Optional[ClauseListEntry]]:
    ...
    for attempt in range(2):
        try:
            ...
            if is_regenerate and prior_draft_for_prompt is not None:
                _validate_regenerated_draft_differs(raw.versions[0], prior_draft_for_prompt)
...
async def generate_describe_draft(
    ...
    clear_prior = detected_agreement_type is not None and stored_agreement_type is not None and detected_agreement_type.lower() != stored_agreement_type.lower()
    ...
    if ignore_document:
        has_document = False
        logger.info(
            "describe_draft session=%s ignore_document=true — forcing no-doc path " "even though session may have an attached document",
            session_id,
        )
    else:
        ...
        if has_document:
            ...
            if not effective_regenerate and relevant_chunks:
                try:
                    ...
                    logger.warning("duplicate check errored session=%s error=%s", session_id, str(e))
                    existing = None
                if existing is not None:
                    ...
                    section_heading = existing.location.section_heading if existing.location and existing.location.section_heading else None
                    ...
                    if existing.summary and existing.summary.strip():
                        seed_summary = f"{existing.summary.strip()} " f"(Existing clause from the uploaded document{location_suffix}.)"
                    else:
                        seed_summary = (
                    ...
                    if auto_version is not None:
                        ...
                        logger.info(
                            "describe_draft_audit session=%s mode=single_clause_exists " "agreement_type=%s similarity=%.3f auto_drafted=true " "latency_ms=%d",
                            session_id,
    ...
    is_regenerate_hit = mode == "single_clause" and regenerate and stored_last_version is not None
    ...
    latency_ms = int((time.time() - start_time) * 1000)
    logger.info(
        "describe_draft_audit session=%s mode=%s agreement_type=%s " "units_generated=%d regenerate=%s grounded=%s latency_ms=%d",
        session_id,
        mode,
        effective_agreement_type or "unknown",
        units_generated,
...
async def _regenerate_by_title(
    ...
    user_prompt_for_llm = (f'Regenerate the clause titled "{prior_draft.title}". ' + (f"Refinement: {refinement_clean}" if refinement_clean else "")).strip()
    ...
    latency_ms = int((time.time() - start_time) * 1000)
    logger.info(
        "describe_draft_audit session=%s mode=single_clause target_title=%s " "regenerate=True grounded=%s latency_ms=%d",
        session_id,
        target_clause_title,
        grounded_in_doc,
        latency_ms,
