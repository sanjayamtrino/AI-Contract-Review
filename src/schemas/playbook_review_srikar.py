...
class RuleCheckRequest(BaseModel):
    ...
    textinformation: Optional[List[TextInfo]] = Field(
        default=None,
        description=(
            "Paragraphs to review. Optional — when omitted or empty, the "
            "agent falls back to the paragraphs stored in the session's "
            "chunk_store (populated by the ingestion endpoint)."
        ),
    )
...
class PlayBookReviewLLMResponse(BaseModel):
    ...
    matched_clause_name: str = Field(
        default="",
        description=(
            "Concise human-readable name of the clause this rule reviewed in "
            "the contract (e.g. 'Confidentiality', 'Term and Survival', "
            "'Governing Law'). Used as a fallback label when the regex/"
            "metadata title-matcher couldn't anchor the rule. Max 5 words. "
            "Empty string if status is 'Not Found'."
        ),
    )
    ...
    suggested_fix: str = Field(
        ...,
        description=(
            "Exact replacement clause text. The frontend substitutes this "
            "verbatim into the contract — no commentary, prefix, or markdown. "
            "Empty string when status is Good."
        ),
    )
...
class PlayBookReviewResponse(BaseModel):
    ...
    matched_clause_titles: List[str] = Field(
        default_factory=list,
        description="Clause heading(s) the rule was matched against in the contract.",
    )
    match_strategy: str = Field(
        default="",
        description="How the clause was selected: title_fuzzy, title_embedding, or content_fallback.",
    )
