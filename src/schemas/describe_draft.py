...
class DescribeDraftRequest(BaseModel):
    ...
    prompt: Optional[str] = Field(
        default=None,
        description=("Free-text drafting request (e.g. 'draft an NDA' or 'draft a liquidity cap clause'). " "Optional when `target_clause_title` is supplied."),
        max_length=2000,
    )
    regenerate: bool = Field(
        default=False,
        description=("If true and the session has a prior draft (single_clause or list_of_clauses), " "produce an improved variation instead of a fresh draft."),
    )
    ...
    def _require_prompt_or_target(self) -> "DescribeDraftRequest":
        ...
        if prompt_empty and target_empty:
            raise ValueError("Either 'prompt' or 'target_clause_title' must be provided.")
...
class ClauseListEntry(BaseModel):
    ...
    drafted_clause: str = Field(description=("Full drafted clause text ready to drop into the agreement. " "Contains `[PLACEHOLDER]` tokens when no document is attached."))
    placeholders: List[str] = Field(
        default_factory=list,
        description=("Distinct placeholder tokens used in drafted_clause " "(e.g. ['[PARTY A]', '[EFFECTIVE DATE]']). Empty when doc-grounded."),
    )
...
class ClauseVersion(BaseModel):
    ...
    placeholders: List[str] = Field(
        default_factory=list,
        description=("Distinct placeholder tokens used in drafted_clause. Empty when doc-grounded."),
    )
...
class DuplicateCheckResult(BaseModel):
    ...
    is_duplicate: bool = Field(description=("True if the candidate text is already a clause on the same topic " "as the user's drafting request."))
    ...
    summary: Optional[str] = Field(
        default=None,
        description=(
            "2-3 sentence plain-English explanation of what the matched clause " "actually does — scope, allocation of risk, and notable carve-outs. " "Only populated when is_duplicate is true."
        ),
    )
...
class ExistingClauseMatch(BaseModel):
    ...
    drafted_clause: str = Field(
        description=(
            "Verbatim text of the existing clause as pulled from the document. " "Field is named drafted_clause to match the shape of freshly-drafted " "single_clause and list_of_clauses responses."
        ),
    )
    similarity_score: float = Field(description="Retrieval similarity score of the matched chunk.")
    location: Optional[ClauseLocation] = Field(
        default=None,
        description=("Where the matching clause sits in the document — chunk index, " "nearest heading, and page number when available."),
    )
...
class DescribeDraftResponse(BaseModel):
    ...
    existing_clause: Optional[ExistingClauseMatch] = Field(
        default=None,
        description=("Populated only in single_clause_exists mode — the matching clause " "already present in the uploaded document."),
    )
    ...
    grounded_in_document: bool = Field(
        default=False,
        description=("True when a document was attached to the session and the draft was grounded " "in it (parties, governing law, and relevant existing clauses)."),
    )
