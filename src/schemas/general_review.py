"""Schemas for the General Review Agent."""
...
class Suggestion(BaseModel):
    ...
    clause_title: str = Field(
        description="Title/heading of the clause this suggestion applies to"
    )
    reason: str = Field(
        description="Plain-language justification for the change, grounded in the clause text"
    )
    original_text: str = Field(
        description="Exact verbatim substring of the clause text to be replaced"
    )
    suggested_fix: str = Field(
        description="Proposed replacement text for original_text"
    )
...
class PromptSplitLLMResponse(BaseModel):
    ...
    subtopics: List[str] = Field(
        default_factory=list,
        description=(
            "Atomic sub-instructions extracted from the user prompt. "
            "Each element is one self-contained contract-review ask."
        ),
    )
...
class RelevanceCheckLLMResponse(BaseModel):
    ...
    relevant: bool = Field(
        description="True if the user's query applies to the selected clause"
    )
    reason: str = Field(
        description=(
            "Short explanation for the user. When relevant=false, this is "
            "the alert message telling them why the clause and query don't match."
        )
    )
...
class GeneralReviewRequest(BaseModel):
    ...
    prompt: str = Field(
        description="User's review instruction (e.g., 'Check for unfair liability terms')"
    )
...
class GeneralReviewResponse(BaseModel):
    ...
    mode: Literal["clause", "document"] = Field(
        description="'clause' if the user selected a clause, 'document' for full review"
    )
